...
public class UserManageDao {
	...
	public List<User> showUser(){
		...
		try {
			List<User> users = queryRunner.query(con, "select  uid,uname,upassword,realname,uaddress,uemail,utelephone,uidnumber,ustate from `user` where `user`.uid in (select upload.uid from upload where upload.state = 2 group by upload.uid ) and `user`.ustate=1",new BeanListHandler<>(User.class));
			return users;
