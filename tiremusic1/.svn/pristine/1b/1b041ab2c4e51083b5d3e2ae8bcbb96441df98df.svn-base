...
public class Statisticscontroller extends BaseController{
	...
	public void showDownNumber(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//不分页返回
		list = ss.getDownNum();
		int pageNum = 1;//Integer.parseInt(req.getParameter("pagenum"));
		int pageSize = 2;//Integer.parseInt(req.getParameter("pagesize"));
		//分页返回
...
//		req.getRequestDispatcher("/chahua/listen.jsp").forward(req, resp);
		resp.getWriter().write(JSONArray.fromObject(list).toString());
	}
	/**
	 * 获取注册用户的数量
